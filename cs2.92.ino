...
// WiFi 连接信息
const char *ssid = "chaeng"; // WIFI名称
const char *password = "19792tiger221"; // WIFI密码
const char *host = "192.168.1.2";//电脑IP地址
...
void drawHalfCircleBar(int x, int y, int radius, int percentage, const char *label)
{
    ...
    // 绘制半圆
    u8g2.drawArc(x, y, radius, 0, angle);

    // 绘制文本标签
    u8g2.setFont(u8g2_font_ncenB08_tr);
    u8g2.setCursor(x - radius, y + radius + 2);
    u8g2.print(label);
}
...
void displayData()
{
    ...
    u8g2.print(cpu_usage);
    u8g2.print("%");

    u8g2.setCursor(0, 20);
    u8g2.print("RAM usage: ");
    u8g2.print(ram_usage);
    u8g2.print("%");

    u8g2.setCursor(0, 30);
    u8g2.print("GPU usage: ");
    u8g2.print(gpu_usage);
    u8g2.print("%");
...
void displayClock()
{
    ...
    // 获取当前时间（通过NTP同步）
    unsigned long epochTime = timeClient.getEpochTime();  // 使用timeClient获取当前时间
